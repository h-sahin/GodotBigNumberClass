...
# HACK: This dictionary is inefficient, along with to_aa().
...
static func add(x, y) -> Big:
    ...
    elif x.is_less_than(y):  # When difference between values is too big, discard the smaller number
...
static func power(x: Big, y) -> Big:
    ...
    if typeof(y) == TYPE_FLOAT:
        ...
        if round(y) != y:
            printerr(
                "Big Error: Power function does not support large floats, use integers!"
            )
...
func to_logarithmic(no_decimals_on_small_values = false) -> String:
        ...
        if split.size() > 1:
            ...
            return (
                ...
                + split[1].substr(
                    0,
                    min(
                        options.logarithmic_decimals,
                        (
                            options.dynamic_numbers - split[0].length()
                            if options.dynamic_decimals
                            else options.logarithmic_decimals
                        )
                    )
                )
            )
    ...
    dec = dec.substr(
        ...
        min(
            options.logarithmic_decimals,
            (
                options.dynamic_numbers - formated_exponent.length()
                if options.dynamic_decimals
                else options.logarithmic_decimals
            )
        )
